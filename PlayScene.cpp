...
//				delete imgTarget;
	...
	// TODO 2 (8/8): Enable the creation of the 4th enemy.
        case 4:
            EnemyGroup->AddNewObject(enemy = new MimiEnemy(SpawnCoordinate.x, SpawnCoordinate.y));
			break;
		default:
			continue;
		}
...
void PlayScene::UIBtnClicked(int id) {
    ...
    else if (id == 3 && money >= MimiTurret::Price)
		preview = new MimiTurret(0, 0);
...
std::vector<std::vector<int>> PlayScene::CalculateBFSDistance() {
	...
	while (!que.empty()) {
        ...
        if (p.x+1<MapWidth&&mapState[p.y][p.x+1]==TILE_DIRT) {
            if (map[p.y][p.x+1]==-1) {
                map[p.y][p.x+1] = map[p.y][p.x] + 1;
                que.push(Engine::Point(p.x+1, p.y));
            }
        }
        if ((p.x)-1>=0&&mapState[(p.y)][p.x-1]==TILE_DIRT) {
            if (map[p.y][p.x-1]==-1) {
                 map[p.y][p.x-1] = map[p.y][p.x] + 1;
                que.push(Engine::Point(p.x-1, p.y));
            }
        }
        if (p.y-1>=0&&mapState[p.y-1][p.x]==TILE_DIRT) {
            if (map[p.y-1][p.x]==-1) {
                map[p.y-1][p.x] = map[p.y][p.x] + 1;
                que.push(Engine::Point(p.x, p.y-1));
            }
        }
        if (p.y+1<MapHeight&&mapState[p.y+1][p.x]==TILE_DIRT) {
            if (map[p.y+1][p.x]==-1) {
                map[p.y+1][p.x] = map[p.y][p.x] + 1;
                que.push(Engine::Point(p.x, p.y+1));
            }
